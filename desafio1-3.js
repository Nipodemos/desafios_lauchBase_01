...
console.log("Exercício usuários e tecnologias");
...
for (let index = 0; index < usuarios.length; index++) {
  ...
  let frase = `${usuario.nome} trabalha com `;

  for (let i = 0; i < usuario.tecnologias.length; i++) {
    const tecnologia = usuario.tecnologias[i];
    frase += `${tecnologia} `
  }
...
console.log("Mostrando todos os usuarios que trabalham com CSS");
...
clientes.forEach((usuario) => {
  ...
  let tipo = saldo < 0 ? "NEGATIVO" : "POSITIVO"
  console.log(`${usuario.nome} possui saldo ${tipo} de ${saldo}`);
})
...
//a soma deles e use-a para calcular a soma de receitas e despesas dentro da função calculaSaldo:

function somaNumeros(numeros) {
  // Soma todos números dentro do array "numeros"
  const numbers = Array.from(numeros)
  let soma = 0
  soma = numbers.reduce((previous, current) => previous + current)
  return soma
}
